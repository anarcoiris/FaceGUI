...
def make_requests_session(cfg=None):
    ...
    if retries and retries > 0:
        retry = Retry(total=retries, backoff_factor=backoff,
                      status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["GET","POST","PUT","DELETE","HEAD","OPTIONS"])
        adapter = HTTPAdapter(max_retries=retry)
        session.mount('https://', adapter)
        session.mount('http://', adapter)
    return session

# Auth header builder (from config dict)
def get_auth_headers_from_cfg(cfg):
    # cfg: dict with keys 'auth_method', 'key', 'use_managed_identity'
    if cfg.get('auth_method') == 'Azure AD':
        try:
            from azure.identity import DefaultAzureCredential
        except Exception as e:
            raise RuntimeError('azure-identity no instalada. Ejecuta: pip install azure-identity')
...
def do_request_core(cfg, method, path, params=None, headers=None, data=None, json_payload=None, stream=False):
    ...
    auth_h = get_auth_headers_from_cfg(cfg)
    hdrs = {}
    if headers:
        hdrs.update(headers)
    hdrs.update(auth_h)
    timeout = cfg.get('timeout', 10)
    resp = session.request(method, url, params=params, headers=hdrs, data=data, json=json_payload, timeout=timeout, stream=stream)
...
# Face API core helper functions
def detect_faces_core(cfg, img_bytes, return_landmarks=True, return_face_id=True, detection_model=DEFAULT_DETECTION_MODEL, recognition_model=DEFAULT_RECOGNITION_MODEL, return_face_attributes=None):
    params = {
    ...
    return do_request_core(cfg, 'POST', '/detect', params=params, headers={'Content-Type': 'application/octet-stream'}, data=img_bytes)

def verify_faces_core(cfg, faceId1, faceId2):
    payload = {'faceId1': faceId1, 'faceId2': faceId2}
    return do_request_core(cfg, 'POST', '/verify', headers={'Content-Type': 'application/json'}, json_payload=payload)
...
def identify_core(cfg, faceIds, largePersonGroupId, maxNumOfCandidatesReturned=5, confidenceThreshold=0.5):
    ...
    return do_request_core(cfg, 'POST', '/identify', headers={'Content-Type': 'application/json'}, json_payload=payload)
...
def find_similar_core(cfg, faceId, largeFaceListId=None, faceIds=None, maxNumOfCandidatesReturned=5, mode='matchPerson'):
    ...
    if largeFaceListId:
        payload['largeFaceListId'] = largeFaceListId
    if faceIds:
        payload['faceIds'] = faceIds
    return do_request_core(cfg, 'POST', '/findsimilars', headers={'Content-Type': 'application/json'}, json_payload=payload)
...
def create_large_person_group_core(cfg, group_id, name, recognition_model=DEFAULT_RECOGNITION_MODEL):
    payload = {'name': name, 'recognitionModel': recognition_model}
    return do_request_core(cfg, 'PUT', f'/largepersongroups/{group_id}', headers={'Content-Type': 'application/json'}, json_payload=payload)
...
def create_person_core(cfg, group_id, person_name, user_data=None):
    ...
    return do_request_core(cfg, 'POST', f'/largepersongroups/{group_id}/persons', headers={'Content-Type': 'application/json'}, json_payload=payload)

def add_face_to_person_core(cfg, group_id, person_id, img_bytes):
    return do_request_core(cfg, 'POST', f'/largepersongroups/{group_id}/persons/{person_id}/persistedFaces', headers={'Content-Type': 'application/octet-stream'}, data=img_bytes)
...
def delete_persisted_face_core(cfg, group_id, person_id, persistedFaceId):
    ...
    headers = get_auth_headers_from_cfg(cfg)
    resp = session.delete(url, headers=headers, timeout=cfg.get('timeout', 10))
    resp.raise_for_status()
    return True
...
def create_liveness_session_core(cfg, mode='active', verify_image_bytes=None):
    ...
    return do_request_core(cfg, 'POST', '/detectLiveness-sessions', headers={'Content-Type': 'application/json'}, json_payload=payload)

def get_liveness_session_core(cfg, session_id):
    return do_request_core(cfg, 'GET', f'/detectLiveness-sessions/{session_id}')

# --------------------- FastAPI app (exposes detect/identify) ---------------------
...
class Creds(BaseModel):
    ...
    auth_method: Optional[str] = None  # 'Subscription Key' or 'Azure AD'
...
def api_detect(req: DetectRequest):
    ...
    try:
        res = detect_faces_core(cfg, img_bytes, return_landmarks=req.returnFaceLandmarks, return_face_id=req.returnFaceId, detection_model=req.detectionModel, recognition_model=req.recognitionModel, return_face_attributes=req.returnFaceAttributes)
        return {'result': res}
    except Exception as e:
...
def api_identify(req: IdentifyRequest):
    cfg = build_cfg_from_creds(req.creds)
    try:
        res = identify_core(cfg, req.faceIds, req.largePersonGroupId, maxNumOfCandidatesReturned=req.maxNumOfCandidatesReturned, confidenceThreshold=req.confidenceThreshold)
        return {'result': res}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

# --------------------- STREAMLIT UI (wrapped to avoid running on import) ---------------------
